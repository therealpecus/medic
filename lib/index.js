...
var requestLimiter = require('request-rate-limiter');
...
function check(opt, callback) {
    ...
    var rateLimiter = new requestLimiter({
         ...
         maxWaitingTime: 1.3 * options.urls.length * options.interval / options.rate
    })
    ...
    urlRequests = options.urls.map(
        function(requestUrl) {
            ...
            rateLimiter.request().then(function(backoff) {
