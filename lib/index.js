...
function check(opt, callback) {
    ...
    var waitTolerance = 1.3;
    var rateLimiter = new requestLimiter({
        rate: options.rate,
        interval: options.interval,
        maxWaitingTime: waitTolerance * options.urls.length * options.interval / options.rate
    });

    urlRequests = options.urls.map(
        function (requestUrl) {
            var jar = request.jar();
            ...
            return rateLimiter.request().then(function () {
                return request.getAsync({
                    ...
                    .then((response) => {
                        ...
                        if (responseUrl.indexOf('500.aspx') >= 0 ||
                            response.body.indexOf('UnhandledException') >= 0) {
    ...
    return Promise.all(urlRequests)
        .then((processedUrls) => {
            ...
            return callback ? callback(null, processedUrls) : processedUrls;
        })
        .catch((error) => {
